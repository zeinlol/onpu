...
begin
 ...
 try
  ...
  While not(eof(file_3)) do
   begin
    ...
    read(File_3,DB_Elem^);
    Main_DB.Add(DB_Elem);
    if DB_Elem^.Type_='типы сетевых операционных систем' then
     begin
       ListBoxOSs.Items.Add(DB_Elem^.Name_);
     end;
    if DB_Elem^.Type_='программное обеспечение' then
       ListBoxSofts.Items.Add(DB_Elem^.Name_);

    if DB_Elem^.Type_='процессоры' then
        ListBoxCPUs.Items.Add(DB_Elem^.Name_);

    if DB_Elem^.Type_='среда передачи' then
     begin
   ...
   // временно
   {New(DB_Elem);
   DB_Elem.Type_ :=  'среда передачи' ;
   DB_Elem.Name_ := 'беспроводная среда передачи';
 ...
 for i:=0 to Konc_List.Count-1 do
  ...
  MessageDlg('Не хватает некоторых или всех файлов для работы программы.'+
    #13+#10+'Обратитесь за соответствующей помощью к разработчику программы'+
    #13+#10+'Возможно Вам необходимо переустановить "OrLAN for Windows 9x/NT/2000."',
...
begin
 ...
 New(DB_Elem);
 DB_Elem^.Type_:='типы сетевых операционных систем';
...
begin
 ...
 New(DB_Elem);
 DB_Elem^.Type_:='программное обеспечение';
...
//Нажата кнопка "добавить" для коммутатора
procedure TFormDB.SpeedButtonAddSwitchClick(Sender: TObject);
var i,j:integer;
    fl:boolean;
begin
 if Komm_List.Count<>0 then
  for i:=0 to Komm_List.Count-1 do // проверяем есть ли в БД
   begin                           // комут. с таким именем
    Komm_Elem:=Komm_list.Items[i];
    if EditSwitchModel.Text=Komm_Elem^.Prod_Name_ then
     begin
      MessageDlg('Такой коммутатор уже имеется в базе данных или Вы ошиблись в названии коммутатора.',
        mtError,[mbOk], 0);
      Exit;
     end;
   end;

  // иначе добавляем коммутатор в БД
 New(Komm_Elem);

 Komm_Elem^.Producer_:=EditSwitchManufacturer.Text; // производитель
 Komm_Elem^.Prod_Name_:=EditSwitchModel.Text; // название
 Komm_Elem^.Int_Bus_:=StrToFloat(EditSwitchBusCapacity.Text); // шина
 Komm_Elem^.Cost_:=StrToInt(EditSwitchPrice.Text); // стоимость

 if CheckBoxSwitchE.Checked then // порты Ethernet
  begin
 ...
 if CheckBoxSwitchFE.Checked then // порты Fast Eth.
  begin
 ...
 Komm_List.Add(Komm_Elem); // записываем  в общую БД с коммутаторами

 ListBoxSwitches.Items.Add(Komm_Elem^.Prod_Name_); // добавляем имя в ListBox
...
begin
 if AP_List.Count<>0 then
  for i:=0 to AP_List.Count-1 do // проверяем есть ли в БД
   begin                           // ТД с таким именем
    AP_Elem:=AP_list.Items[i];
    if EditAPModel.Text=AP_Elem^.Prod_Name_ then
     begin
      MessageDlg('Такая точка доступа уже имеется в базе данных или Вы ошиблись в названии точки доступа.',
        mtError,[mbOk], 0);
      Exit;
     end;
   end;

  // иначе добавляем ТД в БД
 New(AP_Elem);

 AP_Elem^.Producer_:=EditAPManufacturer.Text; // производитель
 AP_Elem^.Prod_Name_:=EditAPModel.Text; // название
 AP_Elem^.Int_Bus_:=StrToFloat(EditAPBusCapacity.Text); // шина
 AP_Elem^.Cost_:=StrToInt(EditAPPrice.Text); // стоимость

 if CheckBoxAPE.Checked then // порты Ethernet
  begin
 ...
 if CheckBoxAPFE.Checked then // порты Fast Eth.
  begin
 ...
 if CheckBoxAPEFE.Checked then // порты 10/100
  AP_Elem^.E_F_Eth_PortNumber_:=StrToInt(EditAPEFEPorts.Text)
 else AP_Elem^.E_F_Eth_PortNumber_:=0;

 if CheckBoxAPWifi.Checked then // беспроводные абоненты
 ...
 AP_List.Add(AP_Elem); // записываем  в общую БД с точками доступа

 ListBoxAP.Items.Add(AP_Elem^.Prod_Name_); // добавляем имя в ListBox
...
var i,j,k:integer;
begin
// показываем коммутатор с параметрами
 View_:=True;
...
var i,j,k:integer;
begin
// показываем точку доступа с параметрами
 View_:=True;
...
begin
 for i:=0 to ListBoxPhLayer.Items.Count-1 do
  if ListBoxPhLayer.Items[i]=Edit11.Text then
   begin
    MessageDlg('Такой протокол уже имеется в базе данных или Вы ошиблись в названии протокола.',
      mtError,[mbOk], 0);
    Edit11.Text:='';
...
begin
 for i:=0 to ListBoxMedia.Items.Count-1 do
  if ListBoxMedia.Items[i]=ComboBox2.Text then
   begin
    MessageDlg('Такая среда передачи уже имеется в базе данных для текущего протокола или Вы ошиблись в названии среды передачи.',
     mtError,[mbOk], 0);
    Exit;
   end;
...
//Показать матрицу задержек переключений
...
//Нажата кнопка "Добавить" для концентратора
procedure TFormDB.SpeedButtonAddHubClick(Sender: TObject);
var i,j:integer;
    fl:boolean;
begin
 if Konc_List.Count<>0 then
  for i:=0 to Konc_List.Count-1 do // проверяем есть ли в БД
   begin                           // конц. с таким именем
    Konc_Elem:=Konc_list.Items[i];
    if EditHubModel.Text=Konc_Elem^.Prod_Name_ then
     begin
      MessageDlg('Такой концентратор уже имеется в базе данных или Вы ошиблись в его названии.',
        mtError,[mbOk], 0);
      Exit;
     end;
   end;

  // иначе добавляем концетратор в БД
 New(Konc_Elem);

 Konc_Elem^.Producer_:=EditHubManufacturer.Text; // производитель
 Konc_Elem^.Prod_Name_:=EditHubModel.Text; // название
 Konc_Elem^.Cost_:=StrToInt(EditHubPrice.Text); // стоимость

 if CheckBoxHubE.Checked then // порты Ethernet
  begin
 ...
 if CheckBoxHubFE.Checked then // порты Fast Eth.
  begin
...
//Нажата кнопка "Изменить" для коммутатора
...
begin
 if Komm_List.Count<>0 then
  ...
  for i:=0 to Komm_List.Count-1 do // проверяем есть ли в БД
   ...
   if fl=false then
    begin
      MessageDlg('Нет такого коммутатора в базе данных',
        mtError,[mbOk], 0);
      Exit;
    end;

  // изменяем коммутатор в БД
 Komm_Elem^.Producer_:=EditSwitchManufacturer.Text; // производитель
 Komm_Elem^.Prod_Name_:=EditSwitchModel.Text; // название
 Komm_Elem^.Int_Bus_:=StrToFloat(EditSwitchBusCapacity.Text); // шина
 Komm_Elem^.Cost_:=StrToInt(EditSwitchPrice.Text); // стоимость
...
//Нажата кнопка "Изменить" для точки доступа
...
begin
 if AP_List.Count<>0 then
  ...
  for i:=0 to AP_List.Count-1 do // проверяем есть ли в БД
   ...
   if fl=false then
    begin
      MessageDlg('Нет такой точки доступа в базе данных',
        mtError,[mbOk], 0);
      Exit;
    end;
 ...
 if CheckBoxAPGE.Checked then // порты Gigabit Ethernet
  AP_Elem^.G_Eth_PortNumber_:=StrToInt(EditAPGEPorts.Text)
 else AP_Elem^.G_Eth_PortNumber_:=0;
 ...
 if RadioButtonAPFullDup.Checked then // конфигурация
  AP_Elem^.Duplex_:=True
 else AP_Elem^.Duplex_:=False;
